...
class Node:
    ...
    def get_node(self, name_nd: str, raise_missing: bool = True) -> Union["Node", any]:
        """
        ...
        if name_nd in self.__dict__.keys():
            ...
            for key in self.__dict__.keys():
                if isinstance(
                    getattr(self, key), Node
                ):  # if sub-Node of self named 'key' has type Node then recursively the get_node function is called
                    ...
                    if temp is None:
                        ...
                        return temp
        if raise_missing:
            raise ValueError("'name_nd' does not exists for this Node!")
    ...
    def to_dict(self, nd_name: str = None) -> dict:
        ...
        new_node: "Node" = self.get_node(nd_name, False) if nd_name is not None else self

        return {key: to_dictionary(val) if isinstance(val, Node) else val for key, val in new_node.__dict__.items()}
    ...
    def glue_nodes(self, other: "Node", to_glue_nd: str, other_name: str):
        ...
        if not isinstance(self.get_node(to_glue_nd), Node):
            raise TypeError("'to_glue_nd' should be a subNode of this Node!")
...
def to_dictionary(node: Node, nd_name: str = None) -> dict[str, any]:
    """
    Transforms a Node to a dictionary.
    ...
    new_node: Node = node.get_node(nd_name) if nd_name is not None else node
