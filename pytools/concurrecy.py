...
def concurrency_manager(engine: str = "thread", lock: bool = False, own_lock: bool = False) -> callable:
    ...
    def outer_wrap(function: callable) -> callable:
        ...
        @wraps(function)
        def inner_wrap(
            args: Union[dict[str, tuple[any]], dict[str, dict[str, any]]],
            kwargs: Union[dict[str, dict[str, any]], None] = None,
...
class ConcurrencyManager:
    ...
    def concurrency_manager(
        ...
        if (args is not None and kwargs is None) or (
            ...
            raise TypeError(
                "args and kwargs must be dictionaries which keys correspond to identifier of"
    ...
    async def async_root(
        self,
        args: dict[str, tuple[any]] | None = None,
        kwargs: dict[str, dict[str, any]] | None = None,
    ) -> None:

        functions = [
            self.single_async(
                args[key] if args is not None else None,
                kwargs[key] if kwargs is not None else None,
            )
...
class MultifunctionConcurrencyManager:
    """
    A class to manage concurrent execution of functions.
